...
def prepro_frame(frame):
    ...
    tf_frame_resized_uint8_batched = tf.expand_dims(tf_frame_uint8, axis=0) # Add batch dimension: (1, x_dim, y_dim, 3)

    return tf_frame_resized_uint8_batched


def object_detection_fcn(model_infer_fcn, frame, params):

    # frame preprocessing for TF inference
    tf_frame_resized_uint8_batched = prepro_frame(frame)
